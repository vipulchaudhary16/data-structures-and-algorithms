# Convert the given infix expression to postfix expression using singly linked list based queue
  
  **hint** : stack using queue
...
{   
    ...
    char top;
           ...
           for(it = infix.begin(); it!=infix.end(); it++) 
           {
              if(char(*it)=='(' || char(*it)=='{' || char(*it)=='[' )
              s1.push(char(*it));
              
              else if(char(*it)==')' || char(*it)=='}' || char(*it)==']')
              {
                if(s1.empty()==0)
                {
                    valid =false;
                ...
                if((top!='(' && char(*it)==')') || (top!='{' && char(*it)=='}') || (top!='[' && char(*it)==']'))
                valid=false;
